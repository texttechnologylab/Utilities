...
<xsl:text>Vertex/.style={ellipse, draw, line width=2.0, fill=white, minimum width=\Scaling*1.75cm},
Seed/.style={ellipse, draw, line width=2.0, fill=SeminarRot, minimum width=\Scaling*2cm},
every label/.style={rectangle callout, draw=SeminarGrau, thin, rounded corners, fill=SeminarHellGrau, text=black, callout relative pointer={(270:0.3cm)}, align=center, minimum width=0.7cm, inner sep=\Scaling*2.5ex, font=\scriptsize\ttfamily},
SeedText/.style={rectangle callout, draw=SeminarGrau, thin, fill=SeminarRot, align=center, minimum width=1cm, font=\ttfamily, text=white, callout relative pointer={(270:0.3cm)}},
...
<xsl:text>\PreviewEnvironment{tikzpicture}
	...
	<xsl:template match="/">		
		...
		<xsl:for-each select="gml:graphml/gml:graph/gml:node">	
			<xsl:choose>
				<xsl:when test="gml:data[@key=$centroidattr_id]/.='true'">
					...
					<xsl:text>)},  Seed, label={[SeedText]above:</xsl:text>
				...
				<xsl:otherwise>
					...
					<xsl:text>,</xsl:text>			
					<xsl:value-of select="gml:data/y:ShapeNode/y:Geometry/@y"/>
					<xsl:text>)},  Vertex, label={above:</xsl:text>
					<xsl:value-of select="gml:data/y:ShapeNode/."/>
					<xsl:text>}]{};</xsl:text>
					<xsl:text>&#xa;</xsl:text>
				</xsl:otherwise>
		...
		<xsl:text>\begin{scope}[SeminarGrau]&#xa;</xsl:text>
		...
		<xsl:text>\end{scope}&#xa;</xsl:text>
		<xsl:text>\begin{scope}[SeminarRot]&#xa;</xsl:text>
