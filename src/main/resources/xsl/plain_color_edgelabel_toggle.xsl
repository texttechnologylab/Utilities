...
<xsl:text>Vertex/.style={circle, line width=2.0, fill=white, minimum size=\Scaling*1.5cm},
EdgeLabel/.style={text opacity=1, font=\scriptsize\ttfamily},
...
<xsl:text>\PreviewEnvironment{tikzpicture}
	...
	<xsl:template match="/">		
		...
		<xsl:for-each select="gml:graphml/gml:graph/gml:node">	
			...
			<xsl:value-of select="gml:data/*/y:Fill/@color"/>
		...
		<xsl:for-each select="gml:graphml/gml:graph/gml:edge">	
			<xsl:text>\path [->,line width=\EdgeScaling*</xsl:text>
			<xsl:value-of select="gml:data/y:PolyLineEdge/y:LineStyle/@width"/>
			<xsl:text>,draw={</xsl:text>
			<xsl:value-of select="gml:data/y:PolyLineEdge/y:LineStyle/@color"/>
			<xsl:text>},](</xsl:text>
			<xsl:value-of select="@source"/>
			<xsl:text>) to </xsl:text>
			<xsl:text>node [midway, auto, EdgeLabel]	{</xsl:text>
			<xsl:value-of select="gml:data/y:PolyLineEdge/."/>
			<xsl:text>} (</xsl:text>
