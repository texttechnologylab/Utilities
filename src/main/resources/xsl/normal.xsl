...
<xsl:text>Vertex/.style={ellipse, draw=VertexBaseDraw, line width=2.0*\Scaling, anchor=west},
every label/.style={align=center, %minimum width=0.7cm, rectangle
inner sep=1, %font=\scriptsize\ttfamily, %draw
font=\Huge\ttfamily, fill=SeminarSehrHellGrau, text=black},
TTLoop/.style={loop, min distance=15mm, looseness=12},%, out=90+25, in=90-25},
every to/.style={bend left}, x=\Scaling,y=\Scaling%, &gt;=Latex
</xsl:text>
    </xsl:variable>
...
%%
\colorlet{VertexBaseCol}{SeminarGruen}
\colorlet{VertexBaseDraw}{SeminarHellGrau}
\colorlet{EdgeBaseCol}{SeminarBlau}
</xsl:text>
    </xsl:variable>
    ...
    <xsl:variable name="newcolors">

        <xsl:for-each select="gml:graphml/gml:graph/gml:node/gml:data/*/y:Fill">
            <xsl:text>\definecolor{</xsl:text>
            <xsl:value-of select="substring(@color,2,7)"/>
        ...
        <!--xsl:for-each select="distinct-values(//gml:graphml/gml:graph/gml:node/gml:data/*/y:Fill/@color)">
            <xsl:text>\definecolor{</xsl:text>
            <xsl:value-of select="@color"/>
            <xsl:text>}{HTML}{</xsl:text>
            <xsl:value-of select="substring(current-grouping-key(),2,7)"/>
            <xsl:text>}</xsl:text>
            <xsl:text>&#xa;</xsl:text>
        </xsl:for-each-->


        <xsl:for-each select="gml:graphml/gml:graph/gml:edge/gml:data/*/y:LineStyle">
        ...
        <!--xsl:for-each select="gml:graphml/gml:graph/gml:edge">
            <xsl:text>\definecolor{</xsl:text>
            <xsl:value-of select="substring(gml:data/*/y:LineStyle/@color,2,7)"/>
            <xsl:text>}{HTML}{</xsl:text>
            <xsl:value-of select="substring(gml:data/*/y:LineStyle/@color,2,7)"/>
            <xsl:text>}</xsl:text>
            <xsl:text>&#xa;</xsl:text>
...
<xsl:text>\PreviewEnvironment{tikzpicture}
    ...
    <xsl:template match="/">
        ...
        <xsl:for-each select="gml:graphml/gml:graph/gml:node">
            ...
            <xsl:text>)},  Vertex, label={\Labelposition:</xsl:text>
            <xsl:value-of select="gml:data/*/."/>
            <xsl:text>},</xsl:text>
            <xsl:text>minimum height={\Scaling*</xsl:text>
            <xsl:value-of select="gml:data/*/y:Geometry/@height"/>
            <xsl:text>},</xsl:text>
            <xsl:text>minimum width={\Scaling*</xsl:text>
            <xsl:value-of select="gml:data/*/y:Geometry/@width"/>
            <xsl:text>},</xsl:text>
            <xsl:text>fill=</xsl:text>
            <xsl:value-of select="substring(gml:data/*/y:Fill/@color,2,7)"/>
            <xsl:text>]{};</xsl:text>
            <xsl:text>&#xa;</xsl:text>
        </xsl:for-each>

        <xsl:text>\begin{scope}[on background layer]\begin{scope}[EdgeBaseCol]</xsl:text>
        <xsl:text>&#xa;</xsl:text>
        <xsl:for-each select="gml:graphml/gml:graph/gml:edge[@source!=@target]">
            ...
            <xsl:value-of select="substring(gml:data/y:PolyLineEdge/y:LineStyle/@color,2,7)"/>
            <xsl:text>](</xsl:text>
        ...
        <xsl:text>\end{scope}</xsl:text>
        <xsl:text>&#xa;</xsl:text>

        <xsl:text>\begin{scope}[EdgeBaseCol]</xsl:text>
        <xsl:text>&#xa;</xsl:text>
        <xsl:for-each select="gml:graphml/gml:graph/gml:edge[@source=@target]">
            ...
            <xsl:text>](</xsl:text>
            <xsl:value-of select="@source"/>
            <xsl:text>.west) to[TTLoop] (</xsl:text>
            <xsl:value-of select="@target"/>
