...
<xsl:text>Vertex/.style={circle, draw=VertextBaseDraw, line width=2.0, fill=white},
...
<xsl:text>\PreviewEnvironment{tikzpicture}
	...
	<xsl:template match="/">		
		...
		<xsl:for-each select="gml:graphml/gml:graph/gml:node">	
			...
			<xsl:text>},</xsl:text>
			<xsl:text>minimum size={\Scaling*</xsl:text>
			<xsl:value-of select="gml:data/*/y:Geometry/@height"/>
			<xsl:text>},</xsl:text>
			<xsl:text>fill={VertexBaseCol}</xsl:text>
			<xsl:text>]{};</xsl:text>
			<xsl:text>&#xa;</xsl:text>
		</xsl:for-each>
		...
		<xsl:text>\end{scope}\end{scope}</xsl:text>
