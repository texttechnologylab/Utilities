...
<xsl:text>Vertex/.style={circle, draw=none, fill=SeminarHellGruen, minimum size=\Scaling*1.95cm},
every label/.style={rectangle, fill=SeminarHellGruen, align=center, minimum width=0.45cm, inner sep=2, text depth=0.25ex, text height=1.25ex, font=\scriptsize\ttfamily},
x=\Scaling,y=\Scaling,
...
<xsl:text>\PreviewEnvironment{tikzpicture}
	...
	<xsl:template match="/">		
		...
		<xsl:text>\begin{scope}[SeminarBlau]&#xa;</xsl:text>
